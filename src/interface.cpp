#include "interface.h"
 ...
 {
    mtx.lock();
    height = msg->data.at(0); width = msg->data.at(1); reso = msg->data.at(2);/* current.x = msg->data.at(3); current.y = msg->data.at(4);*/
    grid.resize(0);
...
void draw(){	
	...
	for (int i = 0; i < height; i++){
			for (int j = 0; j < width; j++){
				...
				for(int i=current.y - 5; i< current.y + 6; i++){
					for(int j=current.x - 5; j< current.x + 6; j++){
						SDL_RenderDrawPoint( ren, dis_x + j, dis_y + i );
					}
				}*/
